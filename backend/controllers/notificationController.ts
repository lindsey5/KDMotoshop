...
export const get_admin_notifications =  async(req: AuthenticatedRequest, res: Response) => {
    try{
        const page = parseInt(req.query.page as string) || 1;
        const limit = parseInt(req.query.limit as string) || 10;
        const skip = (page - 1) * limit;
        const [notifications, totalNotifications, totalUnread] = await Promise.all([
            ...
            .sort({ createdAt: -1})
            .populate('from'),
            await AdminNotification.countDocuments({ to: req.user_id}),
            await AdminNotification.countDocuments({ to: req.user_id, isViewed: false})
        ])
...
export const update_admin_notification =  async(req: AuthenticatedRequest, res: Response) => {
    try{
